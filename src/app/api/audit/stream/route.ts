...
- lookup_particle_data: [MCP — pp-mcp-server (ParticlePhysics MCP)] Offline PDG particle database with 400+ particles. Tools: search_particle, get_data, decay_analysis, error_analysis. Install: pip install particlephysics-mcp (provides pp-mcp-server binary). Falls back to Gemini googleSearch.
- run_quantum_simulation: [MCP — PsiAnimator-MCP + scicomp-quantum-mcp] Priority: PsiAnimator (QuTip-based, create_quantum_state/evolve_quantum_system/measure_observable/quantum_gate_sequence/calculate_entanglement) then scicomp-quantum-mcp (Schrödinger solver, 12 tools) then Gemini. Install PsiAnimator: pip install psianimator-mcp.
- run_symbolic_math: [MCP — scicomp-math-mcp] Real symbolic algebra via SymPy: symbolic_integrate, symbolic_diff, symbolic_solve, symbolic_simplify, matrix ops (14 tools). Install: pip install scicomp-math-mcp or uvx scicomp-math-mcp. Falls back to Gemini.
- run_molecular_dynamics: [MCP — scicomp-molecular-mcp] Real MD simulations: create_particles then run_md/run_nvt, RDF, MSD, thermodynamic analysis (15 tools). Install: pip install scicomp-molecular-mcp or uvx scicomp-molecular-mcp. Falls back to Gemini.
- run_neural_network: [MCP — scicomp-neural-mcp] Real neural network definition: resnet18, mobilenet, or custom architectures; model summary, training workflow (14 tools). Install: pip install scicomp-neural-mcp or uvx scicomp-neural-mcp. Falls back to Gemini.
