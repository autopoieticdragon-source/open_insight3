        ...
        {sortedForums.map((forum) => (
          <Link key={forum.slug} href={`/forums/${forum.slug}`} className="glass-card p-6 block group">
            ...
            <div className="flex items-center gap-4 mb-4 text-xs text-[var(--text-muted)]">
              <span className="hover:text-[var(--accent-indigo)] transition-colors underline decoration-dotted underline-offset-2 cursor-pointer" title="Total discussion threads in this forum category — click to view all threads" aria-label={`View all ${forum.threadCount} threads in ${forum.name}`}>
